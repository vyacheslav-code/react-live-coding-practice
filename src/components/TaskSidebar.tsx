...
export default function TaskSidebar({ tasks, currentTaskId }: TaskSidebarProps) {
  ...
  // Toggle body class for layout margin
  useEffect(() => {
  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed top-20 z-30 w-8 h-8 bg-gray-800 border border-gray-700 rounded-full flex items-center justify-center text-gray-400 hover:text-white hover:bg-gray-700 transition-all duration-300 ${
      ...
      {/* Sidebar */}
      <aside
        className={`fixed left-0 top-0 h-full bg-gray-900 border-r border-gray-800 z-20 transition-all duration-300 ${
      ...
      <div
        ...
        className="overflow-y-auto h-[calc(100vh-65px)] scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-transparent"
      >
        <nav className="py-2">
          {tasks.map((task) => {
